...
- **跟讀錄音**:每句旁邊有麥克風鍵,錄下你自己跟讀的聲音,錄完立即可播放比對(用瀏覽器 MediaRecorder API,不需要後端、不需要語音辨識服務,iPhone Safari 也支援)
...
- 跟讀錄音用 `MediaRecorder` API,錄音檔只存在瀏覽器記憶體(Blob URL),
  重新整理頁面或切換集數就會消失,不會上傳到任何地方,單純是「錄完馬上
  回放比對」的練習工具,不是要長期保存的錄音檔。第一次使用瀏覽器會跳出
  麥克風權限請求,要按允許才能用。
